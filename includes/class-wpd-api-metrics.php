...
final class WPD_Api_Metrics {
	...
	public static function render_admin_health(): void {
		...
		<div class="notice notice-info wpd-api-health">
			...
			<p>
				...
				echo esc_html(
					sprintf(
						/* translators: 1: API calls, 2: cache hits, 3: cache misses, 4: cache hit rate, 5: cumulative API milliseconds, 6: average milliseconds, 7: slowest milliseconds. */
						__( '%1$d appel(s) API — cache %2$d HIT / %3$d MISS (%4$s%%) — API %5$s ms cumulées, %6$s ms en moyenne, %7$s ms au plus lent.', 'wp-piwigo-display' ),
						$metrics['api_calls'],
						$metrics['cache_hits'],
	...
	/**
	 * Computes a compact human-readable health verdict.
	 *
	 * @param array<string, int|float|string> $metrics Aggregated metrics.
	 * @return string
	 */
	private static function health_status( array $metrics ): string {
		if ( '' !== (string) $metrics['last_error'] || 500 <= (int) $metrics['last_http_status'] ) {
			return __( '🔴 Erreur récente détectée', 'wp-piwigo-display' );
		}

		if ( 2000 < (float) $metrics['slowest_ms'] || ( 4 <= ( (int) $metrics['cache_hits'] + (int) $metrics['cache_misses'] ) && 25 > (float) $metrics['cache_hit_rate'] ) ) {
			return __( '🟠 Fonctionnement dégradé à surveiller', 'wp-piwigo-display' );
		}

		if ( 0 === (int) $metrics['api_calls'] && 0 === ( (int) $metrics['cache_hits'] + (int) $metrics['cache_misses'] ) ) {
			return __( '⚪ Aucune activité mesurée pour le moment', 'wp-piwigo-display' );
		}

		return __( '🟢 Fonctionnement normal', 'wp-piwigo-display' );
	}
}
