...
$assert( false !== strpos( $module, "photo_text_line_height']" ) && false !== strpos( $module, 'fitted_font_size(' ) && false !== strpos( $module, 'line_positions(' ), 'Le moteur doit gérer la hauteur de ligne et adapter automatiquement la taille du bloc multiligne.' );
$assert( false !== strpos( $module, 'sanitize_textarea_field' ) && false !== strpos( $module, "'\\\\n'" ), 'Le moteur doit accepter des retours à la ligne réels ou encodés dans le shortcode.' );
$assert( false !== strpos( $module, 'foreach ( $lines as $index => $line )' ) && false !== strpos( $module, 'wpd-photo-text-lines-' ), 'Chaque ligne doit être rendue séparément dans le masque SVG.' );
...
$assert( false !== strpos( $block, '"photoTextSize"' ) && false !== strpos( $block, '"photoTextLetterSpacing"' ) && false !== strpos( $block, '"photoTextLineHeight"' ) && false !== strpos( $block, '"photoTextMaxWidth"' ) && false !== strpos( $block, '"photoTextAlign"' ), 'Les attributs typographiques Gutenberg doivent être déclarés.' );
...
$assert( false !== strpos( $controls, 'TextareaControl' ) && false !== strpos( $controls, 'jusqu’à quatre lignes' ), 'Gutenberg doit proposer une saisie multiligne explicite.' );
$assert( false !== strpos( $controls, 'Taille du texte' ) && false !== strpos( $controls, 'Interlettrage' ) && false !== strpos( $controls, 'Hauteur de ligne (%)' ) && false !== strpos( $controls, 'Largeur maximale (%)' ) && false !== strpos( $controls, 'Alignement' ), 'Gutenberg doit exposer les réglages typographiques.' );
...
$assert( 1 === preg_match( "/'photoTextLineHeight'\\s*=>\\s*'photo_text_line_height'/", $block_php ), 'Le bloc doit transmettre la hauteur de ligne au shortcode.' );
...
$assert( false !== strpos( $classic, '<textarea data-wpd="photo_text"' ) && false !== strpos( $classic, 'photo_text_line_height' ) && false !== strpos( $classic, "replace(/\\r?\\n/g, '\\\\n')" ), 'Classic Editor doit préserver les lignes dans un shortcode compact.' );
...
$assert( false !== strpos( $composer, '<textarea id="wpd-c-photo-text"' ) && false !== strpos( $composer, 'photo_text_line_height' ), 'Le composeur doit exposer le texte multiligne et sa hauteur de ligne.' );
